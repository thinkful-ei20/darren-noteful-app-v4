...
describe('Noteful API - Folders', function () {
  ...
  beforeEach(function () {
    return Promise.all(seedUsers.map(user => User.hashPassword(user.password)))
      .then(digest => {
        seedUsers.forEach((user,i) => user.password = digest[i]);
        return Promise.all([
  ...
  describe('GET /api/folders', function () {
    ...
    it('should return the correct number of folders', function () {
      ...
      const apiPromise = chai.request(app)
        .get('/api/folders').set('Authorization', `Bearer ${token}`);
  ...
  describe('PUT /api/folders/:id', function () {
    ...
    it('should update the folder', function () {
      ...
      return Folder.findOne().select('id name userId')
        .then(_data => {
          data = _data;
          return chai.request(app)
            .put(`/api/folders/${data.id}`).set('Authorization', `Bearer ${token}`)
            .send(updateItem);
        })
        .then(function (res) {
          expect(res).to.have.status(200);
    ...
    it('should return an error when given a duplicate name', function () {
      ...
      return Folder.find({userId: '333333333333333333333300'}).select('id name userId').limit(2)
        .then(results => {
